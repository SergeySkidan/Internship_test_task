...
export function getClients() {
  ...
  xhr.open('GET', 'clients.json', false); // 3. Отсылаем запрос

  xhr.send(); // 4. Если код ответа сервера не 200, то это ошибка
...
export function searchClients(query) {
  ...
  return filterClients; // return axios.get('http://localhost:3001/clients?q='+ query)
  //   .then(response => {
  //     store.dispatch(getClientsSuccess(response.data));
  //     return response;
  //   });
